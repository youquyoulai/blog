...
async function walineFetch(path, method, body) {
  ...
  const text = await res.text();
  if (!res.ok) {
    throw new Error(`Waline API ${res.status}: ${text.substring(0, 200)}`);
  }
  return JSON.parse(text);
}
...
async function listComments(request, env) {
  ...
  const [allData, waitingData, approvedData, spamData] = await Promise.all([
    walineFetch(`/comment?type=list&page=1&pageSize=1&status=all`, 'GET').catch(() => ({ total: 0, data: [] })),
    walineFetch(`/comment?type=list&page=1&pageSize=1&status=waiting`, 'GET').catch(() => ({ total: 0, data: [] })),
    walineFetch(`/comment?type=list&page=1&pageSize=1&status=approved`, 'GET').catch(() => ({ total: 0, data: [] })),
    walineFetch(`/comment?type=list&page=1&pageSize=1&status=spam`, 'GET').catch(() => ({ total: 0, data: [] })),
  ]);
